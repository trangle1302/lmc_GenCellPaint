...
class VQModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            # autoencode
            aeloss, log_dict_ae = self.loss(qloss, y, ypred, optimizer_idx, self.global_step,
        ...
        if optimizer_idx == 1:
            # discriminator
            discloss, log_dict_disc = self.loss(qloss, y, ypred, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), split="train")
    ...
    def _validation_step(self, batch, batch_idx, suffix=""):
        ...
        discloss, log_dict_disc = self.loss(qloss, y, ypred, 1,
                                            self.global_step,
                                            last_layer=self.get_last_layer(),
                                            split="val"+suffix,
                                            predicted_indices=ind
    ...
    def test_step(self, batch, batch_idx, suffix=""):
        ...
        ypred, qloss, ind = self(x, return_pred_indices=True)
        self.log(f"test{suffix} prediction: {ypred.shape}", ypred,
                   prog_bar=True, logger=True, on_step=False, on_epoch=True, sync_dist=True)
